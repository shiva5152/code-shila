...
export default function About() {
  return (
    <section id="about" className="py-28 px-6 bg-[#07050F]">
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {highlights.map((h) => (
                <div
                  key={h.title}
                  className="flex items-start gap-3 p-4 bg-[#0F0A1D] border border-[#1E1535] rounded-xl"
                >
                  <div className="w-9 h-9 rounded-lg bg-[#7C3AED]/15 flex items-center justify-center shrink-0 mt-0.5">
                    <h.icon size={16} className="text-[#7C3AED]" />
                  </div>
                  <div>
                    <p className="text-white text-sm font-semibold mb-0.5">{h.title}</p>
                    <p className="text-[#9CA3AF] text-xs leading-snug">{h.description}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          >
            {/* Glow behind card */}
            <div className="absolute -inset-4 bg-gradient-to-br from-[#7C3AED]/20 to-[#EC4899]/10 rounded-3xl blur-2xl" />

            <div className="relative bg-[#0F0A1D] border border-[#1E1535] rounded-2xl p-8">
              {/* Stats grid */}
              <div className="grid grid-cols-2 gap-6 mb-8">
                {stats.map((s) => (
                  <div key={s.label} className="text-center p-4 bg-[#07050F] rounded-xl border border-[#1E1535]">
              ...
              {/* Current role card */}
              <div className="bg-[#07050F] rounded-xl border border-[#1E1535] p-5">
                <p className="text-[#9CA3AF] text-xs uppercase tracking-widest mb-3">Currently</p>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-[#7C3AED] to-[#EC4899] flex items-center justify-center text-white font-bold text-sm shrink-0">
                    SR
                  </div>
                  <div>
                    <p className="text-white font-semibold text-sm">Software Development Engineer</p>
                    <p className="text-[#9CA3AF] text-xs">Shiprocket · Full-time</p>
                  </div>
                </div>
                <div className="mt-4 flex gap-2 flex-wrap">
                  {["React", "Node.js", "TypeScript", "AWS"].map((tag) => (
                    <span
                      key={tag}
                      className="text-[10px] font-medium text-[#9CA3AF] border border-[#1E1535] rounded-full px-2.5 py-0.5 bg-[#1E1535]/50"
                    >
                      {tag}
                    </span>
                  ))}
